...
class OrdersView(ttk.Frame):
    ...
    def _create_layout(self) -> None:
        ...
        for col, key in zip(("title", "status", "total"), ("orders.add", "orders.status", "orders.total")):
...
class OrderDialog(tk.Toplevel):
    ...
    def _build_form(self) -> None:
        ttk.Label(self, text=i18n.translate("orders.add")).grid(row=0, column=0, sticky="w", padx=12, pady=6)
    ...
    def _on_submit(self) -> None:
        ...
        if not title:
            messagebox.showwarning("Carixon", "Provide title")
            return
